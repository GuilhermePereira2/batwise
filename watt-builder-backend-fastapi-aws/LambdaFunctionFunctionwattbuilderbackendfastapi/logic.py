...
def compute_cell_configurations(req: Any, cell_catalogue: List[CellData], component_db: Dict[str, List[Any]]) -> Dict[str, Any]:
        ...
        for series in range(min_series, max_series + 1): # Inclusivo
            ...
            for parallel in range(min_p_power, MAX_PARALLEL_CELLS):  # Limite aumentado para teste
                ...
                if not safety.is_safe:
                    print(f"Unsafe configuration skipped: {cell.CellModelNo}, {series}S{parallel}P")
                    continue
                # --------------------
                ...
                if tech["needs_fuse"] & req.include_components:
                    ...
                    if not fuse_data:
                        #print(f"Fuse not found for configuration: {cell.CellModelNo}, {series}S{parallel}P")
                ...
                if tech["needs_relay"] & req.include_components:
                    ...
                    if not relay_data:
                        #print(f"Relay not found for configuration: {cell.CellModelNo}, {series}S{parallel}P")
                        continue  # Se precisa e não existe no DB, configuração inválida
                ...
                if tech["needs_shunt"] & req.include_components:
                    ...
                    if not shunt_data:
                        #print(f"Shunt not found for configuration: {cell.CellModelNo}, {series}S{parallel}P")
                        continue  # Se precisa e não existe no DB, configuração inválida
                ...
                if not cable:
                    #print(f"Cable not found for configuration: {cell.CellModelNo}, {series}S{parallel}P")
                    #print(f"Cable Specs Required - Current: {cont_current * FUSE_CURRENT_FACTOR:.1f}, Voltage: {max_voltage:.1f}, Temp: {req.ambient_temp}")
                    continue
                ...
                if not bms:
                    #print(f"BMS not found for configuration: {cell.CellModelNo}, {series}S{parallel}P")
                    continue
    ...
    print(f"Calculation completed. {len(configs)} valid configurations found.")
