...
const SolutionDetailModal = ({ solution, isOpen, onClose, showComponents, dataSource }: { solution: Configuration, isOpen: boolean, onClose: () => void, showComponents: boolean, dataSource: 'default' | 'custom' }) => {
  ...
  const downloadBOM = () => {
    const rows = [['Component', 'Model', 'Quantity', 'Details', 'Unit Price', 'Total Price']];
    const totalCells = solution.series_cells * solution.parallel_cells;
    
    rows.push(['Battery Cells', solution.cell.CellModelNo, totalCells.toString(), `${solution.cell.NominalVoltage}V ${solution.cell.Capacity}mAh`, solution.cell.Price.toFixed(2), (solution.cell.Price * totalCells).toFixed(2)]);
    if (solution.bms) rows.push(['BMS', solution.bms.model, '1', `Max ${solution.bms.a_max}A`, (solution.bms.master_price || solution.bms.price).toFixed(2), (solution.bms.master_price || solution.bms.price).toFixed(2)]);
    if (solution.fuse) rows.push(['Fuse', solution.fuse.model, '1', `${solution.fuse.a_max}A`, solution.fuse.price.toFixed(2), solution.fuse.price.toFixed(2)]);
    if (solution.relay) rows.push(['Relay', solution.relay.model, '1', `${solution.relay.a_max}A`, solution.relay.price.toFixed(2), solution.relay.price.toFixed(2)]);
    if (solution.cable) rows.push(['Cable (2m)', solution.cable.model, '1', `${solution.cable.section}mm²`, solution.cable.price.toFixed(2), solution.cable.price.toFixed(2)]);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-2xl">
            <CircuitBoard className="text-accent" /> System Blueprint
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-4">
          ...
          <div className="space-y-4">
            <div className="bg-slate-50 p-4 rounded-lg">
              <h3 className="font-bold mb-3 flex items-center gap-2"><Battery className="w-4 h-4" /> Core Specs</h3>
              <div className="grid grid-cols-2 gap-y-2 text-sm">
                <span className="text-muted-foreground">Configuration:</span>
                <span className="font-mono font-bold">{solution.series_cells}S {solution.parallel_cells}P</span>
                
                <span className="text-muted-foreground">Nominal Voltage:</span>
                <span>{(solution.series_cells * solution.cell.NominalVoltage).toFixed(1)} V</span>
                
                <span className="text-muted-foreground">Total Capacity:</span>
                <span>{(solution.parallel_cells * solution.cell.Capacity).toFixed(1)} Ah</span>
                
                <span className="text-muted-foreground">Total Energy:</span>
                <span>{formatUnit(solution.battery_energy, 'Wh')}</span>
                
                <span className="text-muted-foreground">Est. Price:</span>
                <span className="text-green-600 font-bold">${solution.total_price.toFixed(2)}</span>
              </div>
            </div>

            {/* Component List */}
            <div className="border rounded-lg p-4">
              <h3 className="font-bold mb-3 text-sm uppercase tracking-wide">Bill of Materials</h3>
              <ul className="space-y-2 text-sm">
                <li className="flex justify-between border-b pb-1">
                  <span>{solution.series_cells * solution.parallel_cells}x {solution.cell.Brand} {solution.cell.CellModelNo}</span>
                  <span className="text-muted-foreground">${(solution.series_cells * solution.parallel_cells * solution.cell.Price).toFixed(2)}</span>
                </li>
                {solution.bms && (
                  <li className="flex justify-between border-b pb-1">
                    <span>BMS: {solution.bms.brand} {solution.bms.model}</span>
                    <span className="text-muted-foreground">${solution.bms.price}</span>
                  </li>
                )}
                {solution.fuse && (
                  <li className="flex justify-between border-b pb-1">
                    <span>Fuse: {solution.fuse.brand} {solution.fuse.model}</span>
                    <span className="text-muted-foreground">${solution.fuse.price}</span>
                  </li>
                )}
              </ul>
            </div>

            <Button className="w-full" onClick={() => requireAuthForDownload(downloadBOM)}>
              <Download className="mr-2 h-4 w-4" /> Download BOM
            </Button>
          </div>
          ...
          <div className="flex flex-col h-full min-h-[300px]">
            ...
            {/* Safety Report */}
            <div className={`mt-4 p-3 rounded-lg border ${solution.safety.is_safe ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
              <div className="flex items-center gap-2 mb-2">
                {solution.safety.is_safe ? <CheckCircle className="text-green-600 w-5 h-5"/> : <AlertTriangle className="text-red-600 w-5 h-5"/>}
                <span className="font-bold text-sm">Safety Score: {solution.safety.safety_score}/100</span>
              </div>
