...
function configGeometryValidation(
  ...
  for (const [e, l] of orientations) {
    ...
    console.log(`[Geometry] Trying orientation e=${e}, l=${l}`);
    console.log(`[Geometry] maxX=${maxX}, maxY=${maxY}, series=${series}, parallel=${parallel}`);
    console.log(`[Geometry] maxConfigX=${maxConfigX}, maxConfigY=${maxConfigY}, nCells=${nCells}`);
    ...
    for (let nx = 1; nx <= maxConfigX; nx++) {
      for (let ny = 1; ny <= maxConfigY; ny++) {
        if (nx * ny == nCells) {
          ...
          console.log(`[Geometry] nx=${nx}, ny=${ny}, totalWidth=${totalWidth}, totalLength=${totalLength}`);
          if (totalWidth <= maxX && totalLength <= maxY) {
            console.log(`[Geometry] PASS: fits in maxX=${maxX}, maxY=${maxY}`);
            found = true;
            return true;
          } else {
            console.log(`[Geometry] FAIL: totalWidth=${totalWidth} > maxX=${maxX} or totalLength=${totalLength} > maxY=${maxY}`);
          }
        }
      }
    }
    if (!found) {
      console.log(`[Geometry] No valid (nx, ny) found for orientation e=${e}, l=${l}`);
    }
  }

  console.log(`[Geometry] FAIL: No orientation fits nCells=${nCells} in maxX=${maxX}, maxY=${maxY}`);
  return false;
}
...
function computeCellConfigurations(
...
): { configs: Configuration[]; stats: Record<string, number> } {
  const configs: Configuration[] = [];

  let totalAttempts = 0;
  let failedHeight = 0;
  let failedGeometry = 0;
  let failedWeight = 0;
  let failedPower = 0;
  let failedEnergy = 0;
  let failedFuse = 0;
  ...
  console.log(`Starting configuration calculation with ${cellCatalogue.length} cells`);
  ...
  for (const cell of cellCatalogue) {
    ...
    // Height validation (cell height + 30mm)
    const requiredHeight = cell.Cell_Height + 30;
    if (requiredHeight > requirements.max_z) {
      failedHeight++;
      continue;
    }
    ...
    console.log(`Checking cell ${cell.CellModelNo}:`);
    console.log(`  Voltage range: ${minSeriesFromVoltage}S - ${maxSeriesFromVoltage}S`);
    console.log(`  Based on: ${requirements.Min_Max_Voltage}V - ${requirements.Max_Max_Voltage}V required`);
    console.log(`  Cell nominal voltage: ${cell.NominalVoltage}V`);

    if (minSeriesFromVoltage > maxSeriesFromVoltage) {
      console.log('  Skipping: No valid series configuration possible');
      continue;
    }
    ...
    for (let series = minSeriesFromVoltage; series <= maxSeriesFromVoltage; series++) {
      ...
      for (let parallel = Math.max(minParallelForEnergy, minParallelForPower, 1); parallel <= 5; parallel++) {
        console.log(`Trying configuration: ${series}S${parallel}P`);
        ...
        if (batteryWeight > requirements.Battery_max_weight) {
          failedWeight++;
          continue;
        }
        ...
        if (batteryEnergy < requirements.Min_Rated_Energy) {
          failedEnergy++;
          continue;
        }
        ...
        // Calculate total price including only available components
        ...
        console.log(`Found valid configuration: ${cell.CellModelNo} - ${series}S${parallel}P`);
  ...
  console.log(`Failed height: ${failedHeight}`);
  console.log(`Failed geometry: ${failedGeometry}`);
  console.log(`Failed weight: ${failedWeight}`);
  console.log(`Failed power: ${failedPower}`);
  console.log(`Failed energy: ${failedEnergy}`);
  console.log(`Failed fuse: ${failedFuse}`);
  console.log(`Failed relay: ${failedRelay}`);
  console.log(`Failed cable: ${failedCable}`);
  console.log(`Failed BMS: ${failedBMS}`);
  console.log(`Failed shunt: ${failedShunt}`);
  console.log(`Failed price: ${failedPrice}`);
  console.log(`Valid configurations: ${configs.length}`);
  ...
  const stats = {
    ...
    failedShunt,
    failedPrice,
    validConfigurations: configs.length,
